...
void drawbar(Monitor *m) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    if ((m->tagset[m->seltags] & 1 << i)) {
      currscheme = tagscheme[i];
      drw_setscheme(drw, currscheme);
    } else {
      drw_setscheme(drw, scheme[SchemeNorm]);
    }
  ...
  if ((w = m->ww - tw - x) > bh) {
    if (m->sel) {
      drw_setscheme(drw, m == selmon ? currscheme : scheme[SchemeSel]);
...
void focus(Client *c) {
  ...
  if (c) {
    ...
    XSetWindowBorder(dpy, c->win, currscheme[1].pixel);
    setfocus(c);
  } else {
